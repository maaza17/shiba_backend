...
router.post('/deleteplace', (req, res) => {
    ...
    placeModel.findOneAndUpdate({_id: _id, is_deleted: false}, {is_deleted: true, is_approved: false}, (err, doc) => {
        if(err){
            return res.status(400).json({
                error: true,
                message: err.message
...
router.post('/restoreplace', (req, res) => {
    placeModel.findOneAndUpdate({_id: _id, is_deleted: true}, {is_deleted: false, is_approved: false}, (err, doc) => {
        if(err){
            return res.status(400).json({
                error: true,
                message: err.message
